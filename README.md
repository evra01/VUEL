...
⚠️ Pour tester l'inscription, ni Twilio ni un serveur SMTP ne sont nécessaires : si tu ne
configures pas `PATCH /admin/sms-config` (ni `PATCH /admin/email-config`), le code OTP
s'affiche directement dans le terminal où tourne la tâche **Backend : lancer en mode debug**
(cherche les lignes commençant par `[SMS non configuré...]` ou `[SMTP non configuré...]`).
Utile pour tester en local ; à retirer avant la prod.

ℹ️ Le code OTP est envoyé par SMS **et** par email : l'adresse email est obligatoire à
l'inscription. L'inscription aboutit si au moins un des deux canaux fonctionne, donc un SMS
qui n'arrive pas ne bloque plus la création de compte. Le SMTP se configure depuis la page
« Email (SMTP) » du back-office admin.
