...
export class WalletService {
  ...
  // Notifie le joueur lui-même une fois le dépôt effectivement crédité (via
  // approveDeposit) ou rejeté — la demande de dépôt elle-même n'a volontairement
  // PAS de notification dédiée (le joueur reste sur l'écran, il voit le lien
  // Wave s'ouvrir immédiatement) : seul le résultat final compte pour lui.
  private async notifyDepositResult(
    transactionId: string,
    walletId: string,
    approved: boolean,
    amount: number,
  ): Promise<void> {
    const wallet = await this.prisma.wallet.findUnique({ where: { id: walletId }, select: { userId: true } });
    if (!wallet) return;
    if (approved) {
      await this.userNotifications.notify(
        ...
        'DEPOSIT_APPROVED',
        'Dépôt crédité ✅',
        `Ton dépôt de ${amount} FCFA a été validé et crédité sur ton solde.`,
      ...
      await this.userNotifications.notify(
        ...
        'DEPOSIT_REJECTED',
        'Dépôt rejeté ❌',
        `Ton dépôt de ${amount} FCFA n'a pas pu être validé — vérifie le paiement Wave et réessaie.`,
        { transactionId, amount },
      );
    }
  }
  ...
  async approveDeposit(transactionId: string) {
    ...
    await this.notifyDepositResult(transactionId, transaction.walletId, true, transaction.amount);
    return updated;
  }
  ...
  async rejectDeposit(transactionId: string) {
    ...
    const updated = await this.prisma.transaction.update({ where: { id: transactionId }, data: { status: 'FAILED' } });
    await this.notifyDepositResult(transactionId, transaction.walletId, false, transaction.amount);
    return updated;
  }
  ...
  async withdraw(userId: string, dto: WithdrawDto) {
    ...
    // Contrairement au dépôt (cf. notifyDepositResult), la DEMANDE de retrait
    // est notifiée dès sa création : contrairement au dépôt, le solde est
    // débité immédiatement et le traitement est manuel côté admin (peut
    // prendre du temps) — le joueur doit savoir que sa demande est bien
    // partie, sans quoi il n'a plus aucun signal avant le résultat final.
    await this.userNotifications.notify(
      userId,
      'WITHDRAWAL_REQUESTED',
      'Demande de retrait envoyée',
      `Ta demande de retrait de ${dto.amount} FCFA a été enregistrée et est en cours de traitement.`,
      { transactionId: transaction.id, amount: dto.amount },
    );
