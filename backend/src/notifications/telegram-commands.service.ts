...
// La légende envoyée avec chaque capture contient "Duel : <code>" — le code
// court (ex: K7QX2M, cf. DuelsService.generateJoinCode) depuis qu'on affiche
// ça plutôt que l'UUID technique (plus simple à relire pour un admin), avec
// repli sur l'UUID pour les quelques duels créés avant l'introduction du
// joinCode (cf. CapturesService.submitProof). On la relit depuis le message
// d'origine (reply_to_message) pour savoir quel duel trancher, sans avoir
// besoin de stocker de correspondance message↔duel côté serveur — puis on la
// résout vers l'id canonique via DuelsService.resolveToId (accepte les deux
// formats).
const DUEL_ID_PATTERN = /Duel\s*:\s*([0-9a-zA-Z-]{4,36})/i;
const WINNER_COMMAND_PATTERN = /^\/gagnant\s+(.+)$/is;

// Commande directe (PAS besoin de répondre à un message précis, contrairement
// à /gagnant ci-dessus) pour rembourser et déverrouiller la mise d'un duel
// par son code court ou son UUID — utile quand un duel reste bloqué avec des
// fonds verrouillés SANS qu'aucune preuve n'ait jamais été envoyée (donc
// aucun message de capture auquel répondre, et le duel n'apparaît pas non
// plus dans la page admin "Preuves", qui ne liste que les duels avec une
// preuve). Accepte /debloquer et /liberer (accents volontairement omis dans
// le nom de la commande — Telegram sur mobile complique la saisie d'accents
// dans un message tapé rapidement).
const UNLOCK_COMMAND_PATTERN = /^\/(debloquer|liberer)\s+(.+)$/is;
...
export class TelegramCommandsService {
  ...
  async handleUpdate(update: any): Promise<void> {
    ...
    const unlockMatch = message.text.match(UNLOCK_COMMAND_PATTERN);
    if (unlockMatch) {
      await this.handleUnlockCommand(message, unlockMatch[2].trim());
      return;
    }
    ...
    const duelRef: string | undefined = message.reply_to_message?.caption?.match(DUEL_ID_PATTERN)?.[1];
    if (!duelRef) {
      await this.telegram.sendMessage(
    ...
    const duelId = await this.duelsService.resolveToId(duelRef);
    if (!duelId) {
      await this.telegram.sendMessage(`⚠️ Duel introuvable (${duelRef}) — a-t-il été supprimé ?`, message.message_id);
      return;
    }
  ...
  /// Traite /debloquer <code> ou /liberer <code> — voir le commentaire sur
  /// UNLOCK_COMMAND_PATTERN. Rembourse les deux joueurs (sans commission) et
  /// annule le duel, exactement comme le bouton "❌ Annuler" envoyé par l'OCR,
  /// mais déclenchable pour N'IMPORTE QUEL duel actif, preuve envoyée ou non.
  private async handleUnlockCommand(message: any, codeOrId: string): Promise<void> {
    const senderId = String(message.from?.id ?? '');
    if (!this.isAdmin(senderId)) {
      await this.telegram.sendMessage("⛔ Tu n'es pas autorisé à débloquer un duel depuis ce compte Telegram.", message.message_id);
      return;
    }

    const duelId = await this.duelsService.resolveToId(codeOrId);
    if (!duelId) {
      await this.telegram.sendMessage(`⚠️ Duel introuvable (${codeOrId}) — vérifie le code.`, message.message_id);
      return;
    }
    ...
    try {
      ...
      if (duel.status === 'COMPLETED' || duel.status === 'CANCELLED') {
        await this.telegram.sendMessage(
          `⚠️ Ce duel est déjà ${duel.status === 'COMPLETED' ? 'réglé' : 'annulé'} — rien à débloquer.`,
          message.message_id,
        );
        return;
      }
      if (duel.tournamentId) {
        await this.telegram.sendMessage(
          "⚠️ Impossible de débloquer un seul match de tournoi ici (les mises sont mutualisées) — annule le tournoi entier depuis la page admin.",
          message.message_id,
        );
        return;
      }
      ...
      await this.telegram.sendMessage(
        '✅ Duel annulé et mises déverrouillées — les deux joueurs ont été remboursés (sans commission).',
        message.message_id,
      );
    } catch (err: any) {
      this.logger.error('Échec du déblocage manuel du duel depuis Telegram:', err);
      await this.telegram.sendMessage(
        `❌ Échec du déblocage : ${err?.message ?? 'erreur inconnue côté serveur'}. Rien n'a été remboursé.`,
        message.message_id,
      );
    }
  }
