...
export class TelegramNotifierService {
  ...
  /// premier accroc.
  /// [targetChatId] permet d'envoyer ailleurs que le canal d'arbitrage des
  /// preuves de score (ex: un canal dédié aux bannières promo, cf.
  /// AdminBannersController) — retombe sur TELEGRAM_CHAT_ID si omis, pour ne
  /// pas casser l'appel existant depuis CapturesService.
  async sendCapturePhoto(
    ...
    if (!token || !chatId) {
      this.logger.warn(
        'TELEGRAM_BOT_TOKEN / TELEGRAM_CHAT_ID (ou TELEGRAM_BANNERS_CHAT_ID) non définies — image non relayée vers Telegram. ' +
          "C'est la cause la plus fréquente d'un échec systématique de l'envoi de preuve/bannière : configurer ces " +
          "variables d'environnement côté back-end (cf. .env.example) avant de réessayer.",
      );
