...
export class AdminController {
  ...
  async approveWithdrawal(@Param('id') id: string) {
    ...
    const updated = await this.prisma.transaction.update({ where: { id }, data: { status: 'SUCCESS' } });
    await this.notifyWithdrawalResult(tx.walletId, 'WITHDRAWAL_APPROVED', 'Retrait effectué ✅', id, tx.amount,
      `Ton retrait de ${tx.amount} FCFA a été envoyé.`);
    return updated;
  }
  ...
  async rejectWithdrawal(@Param('id') id: string, @Body() dto: MarkWithdrawalOtherDto) {
    ...
    const updated = await this.prisma.$transaction(async (txClient: Prisma.TransactionClient) => {
    ...
    await this.notifyWithdrawalResult(tx.walletId, 'WITHDRAWAL_REJECTED', 'Retrait annulé', id, tx.amount,
      `Ton retrait de ${tx.amount} FCFA a été annulé et le montant recrédité sur ton solde.`);
    return updated;
  }
  ...
  async markWithdrawalOther(@Param('id') id: string, @Body() dto: MarkWithdrawalOtherDto) {
    ...
    await this.notifyWithdrawalResult(tx.walletId, 'WITHDRAWAL_OTHER', 'Retrait — statut particulier', id, tx.amount,
      dto?.note
        ? `Ton retrait de ${tx.amount} FCFA suit un traitement particulier : ${dto.note}`
        : `Ton retrait de ${tx.amount} FCFA suit un traitement particulier — contacte le support si besoin.`);
    return updated;
  }
  ...
  // Résout le joueur propriétaire du wallet pour lui notifier le résultat de
  // sa demande de retrait — jamais bloquant : si le wallet est introuvable
  // (ne devrait jamais arriver en pratique), on ignore silencieusement plutôt
  // que de faire échouer l'action admin déjà appliquée en base.
  private async notifyWithdrawalResult(
    walletId: string,
    type: 'WITHDRAWAL_APPROVED' | 'WITHDRAWAL_REJECTED' | 'WITHDRAWAL_OTHER',
    title: string,
    ...
    if (!wallet) return;
    await this.userNotifications.notify(wallet.userId, type, title, message, { transactionId, amount });
  }
