...
export class TournamentsService {
  ...
  async cancel(userId: string, tournamentId: string) {
    ...
    const stillOpen = await this.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
      ...
      const stillOpenDuels = await tx.duel.findMany({
      ...
      return stillOpenDuels;
    });
    // Un duel de bracket annulé ici peut avoir une bulle de capture ouverte
    // côté joueur (match en cours au moment de l'annulation du tournoi) —
    // même notification que pour un duel classique.
    for (const duel of stillOpen) {
      this.duelRoomGateway.notifyDuelResolved(duel.id, 'CANCELLED');
    }
  ...
  async reportMatchResult(duelId: string, winnerId: string) {
    ...
    // Notifie le salon temps réel : sans ça, la bulle de capture Android reste
    // affichée indéfiniment après l'arbitrage d'un duel de tournoi (même bug
    // que pour un duel classique, cf. EscrowService.release/refund).
    ...
    if (loserId) {
      ...
      await this.userNotifications.notify(
        ...
        'Éliminé du tournoi',
        `Tu as été éliminé du tournoi après ce match.`,
        { duelId, tournamentId: duel.tournamentId },
      );
    }
  ...
  private async finalize(tournamentId: string, winnerId: string) {
    ...
    await this.userNotifications.notify(
      ...
      'TOURNAMENT_WON',
      'Tournoi remporté 🏆',
      `Tu as remporté le tournoi "${tournament.name}" ! +${payout} FCFA crédités sur ton solde.`,
      { tournamentId, amount: payout },
    );
  }
}
