...
class DuelsApiClient {
  ...
  /// Duels (OPEN/IN_PROGRESS/DISPUTED) où l'utilisateur courant est un des
  /// deux joueurs — contrairement à list() qui ne renvoie que les salons
  /// encore ouverts à un adversaire. Permet de rouvrir un salon en cours
  /// après en être sorti (cf. PlayTab, section "Tes duels en cours").
  Future<List<dynamic>> listMine() => runApiCall(() async {
        final res = await _send(() => http.get(Uri.parse('$baseUrl/duels/mine'), headers: _headers));
        if (res.statusCode >= 400) throw Exception(_serverErrorMessage(res));
        return jsonDecode(res.body) as List<dynamic>;
      });
