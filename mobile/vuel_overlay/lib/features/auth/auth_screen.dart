...
class _AuthScreenState extends State<AuthScreen> {
  ...
  Future<void> _doRegister() => _run(() async {
        ...
        // Contrôle immédiat côté app pour éviter un aller-retour réseau — le
        // back-end revalide de toute façon (@IsEmail sur RegisterDto).
        if (email.isEmpty) throw Exception('Renseigne ton adresse email');

        final delivery = await _authClient.register(
  ...
  /// Texte affiché au-dessus du champ OTP — on n'annonce que les canaux par
  /// lesquels le code est réellement parti (le serveur peut n'en avoir réussi
  /// qu'un seul), pour éviter de faire attendre un SMS qui n'arrivera jamais.
  String get _otpDestinationLabel {
    final delivery = _otpDelivery;
    final destinations = <String>[];
    if (delivery == null || delivery.sms) destinations.add('par SMS au $_pendingPhone');
    if (delivery != null && delivery.email && delivery.maskedEmail != null) {
      destinations.add('par email à ${delivery.maskedEmail}');
    }
    if (destinations.isEmpty) return 'Code envoyé';
    return 'Code envoyé ${destinations.join(' et ')}';
  }
  ...
  List<Widget> _buildRegisterForm() {
    return [
      ...
      TextField(
        ...
        decoration: const InputDecoration(hintText: 'ton.adresse@exemple.com'),
      ),
      const Padding(
        padding: EdgeInsets.only(top: 4),
        child: Text(
          'Le code de vérification y sera aussi envoyé, au cas où le SMS n\'arrive pas.',
          style: TextStyle(color: VuelColors.muted, fontSize: 11),
        ),
      ),
      const SizedBox(height: 12),
  ...
  List<Widget> _buildOtpForm() {
    return [
      Text(
        _otpDestinationLabel,
        textAlign: TextAlign.center,
        style: const TextStyle(color: VuelColors.muted, fontSize: 13),
      ),
      const SizedBox(height: 4),
      const Text(
        '(mode dev : le code s\'affiche dans les logs Render tant que Twilio/SMTP ne sont pas configurés — onglet "Logs" de ton service sur render.com)',
        textAlign: TextAlign.center,
        style: TextStyle(color: VuelColors.muted, fontSize: 11),
      ),
      const SizedBox(height: 16),
