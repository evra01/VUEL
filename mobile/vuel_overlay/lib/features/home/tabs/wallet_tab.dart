...
class _WalletTabState extends State<WalletTab> {
  ...
  Future<void> _load() async {
    ...
    try {
      // Même correctif que les autres onglets (cf. AccueilTab._load()) : les
      // deux appels démarrent en même temps au lieu de s'enchaîner.
      final walletFuture = widget.walletClient.getWallet();
      final txsFuture = widget.walletClient.getTransactions();
  ...
  /// Montant libre : ouvre d'abord un champ de saisie (min. 200 F, aligné sur
  /// DepositDto.amount côté back-end), puis enchaîne sur le numéro Wave comme
  /// pour un montant prédéfini.
  Future<void> _depositCustomAmount() async {
    final amount = await _promptCustomDepositAmount();
    if (amount == null) return; // annulé
    await _deposit(amount);
  }

  Future<int?> _promptCustomDepositAmount() async {
        ...
        return StatefulBuilder(
          builder: (dialogContext, setDialogState) {
            return AlertDialog(
              ...
              title: const Text('Montant du dépôt', style: TextStyle(color: VuelColors.text, fontSize: 16)),
              content: Column(
                mainAxisSize: MainAxisSize.min,
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  const Text('Montant (FCFA) — minimum 200 F', style: TextStyle(color: VuelColors.muted, fontSize: 12)),
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () {
                    ...
                    if (amount < 200) {
                      setDialogState(() => errorText = 'Montant minimum : 200 F.');
                      return;
                    }
    ...
    return RefreshIndicator(
      ...
      child: ListView(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          // Les boutons ci-dessus ne couvrent que 3 montants fixes — celui-ci
          // laisse le choix d'un montant libre (minimum 200 F), pour ne pas
          // forcer le joueur à déposer plus (ou moins, par paliers) que ce
          // dont il a besoin.
          SizedBox(
            width: double.infinity,
            child: OutlinedButton(
              onPressed: _depositing ? null : _depositCustomAmount,
              child: const Text('Autre montant', style: TextStyle(fontSize: 12)),
            ),
          ),
          const SizedBox(height: 12),
          SizedBox(
