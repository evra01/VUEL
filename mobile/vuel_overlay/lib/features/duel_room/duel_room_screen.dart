...
class _DuelRoomScreenState extends State<DuelRoomScreen> {
  /// Code affiché/copié pour inviter un adversaire — joinCode s'il existe,
  /// sinon l'UUID du duel (cf. resolveDuel côté back-end, qui accepte les
  /// deux). Voir le commentaire au-dessus du bouton "Code du salon" plus bas.
  String get _displayCode => widget.joinCode ?? widget.duelId;
  ...
  void initState() {
    ...
    _socketService.connect(
      ...
      // BUG CORRIGÉ ICI : rien ne fermait la bulle flottante Android une fois
      // la délibération terminée (arbitrage Telegram, page admin, litige ou
      // validation OCR) — le serveur n'émettait jusqu'ici aucun événement à
      // ce moment-là (cf. duel-room.gateway.ts). On arrête la capture dès
      // réception de 'duel_resolved', ce qui fait disparaître la bulle
      // automatiquement, sans action de l'utilisateur.
      onDuelResolved: (status, winnerId) {
        if (!mounted) return;
        setState(() => _status = status);
        unawaited(_captureController.stop());
        final youWon = winnerId != null && winnerId == widget.currentUserId;
        _addSystemEvent(
          status == 'CANCELLED'
              ? 'Duel annulé par l\'arbitrage — mises remboursées.'
              : (winnerId == null
                  ? 'Duel réglé par l\'arbitrage.'
                  : (youWon ? 'Duel réglé — tu as gagné 🏆' : 'Duel réglé — tu as perdu.')),
        );
      },
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          ...
          // Visible tant que le salon attend un adversaire (même condition
          // que l'icône "Partager" de l'AppBar) — permet de copier le code
          // directement, sans repasser par la feuille de partage, pour le
          // recopier à la main ailleurs (message perso, autre appli...).
          //
          // Se rabat sur duelId si joinCode est absent (salon créé avant
          // l'ajout de ce champ — cf. migration 20260917000000_backfill_join_code
          // qui comble ce cas pour les duels existants, gardé ici en filet de
          // sécurité) : mieux vaut un UUID copiable qu'aucun bouton du tout,
          // resolveDuel() côté back-end acceptant les deux formats.
          if (_playerBId == null)
            InkWell(
              onTap: () async {
                await Clipboard.setData(ClipboardData(text: _displayCode));
                if (mounted) VuelFeedback.success(context, 'Code copié : $_displayCode');
              },
              child: Container(
                width: double.infinity,
                color: VuelColors.amberDim,
                padding: const EdgeInsets.symmetric(vertical: 10),
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.center,
                  children: [
                    const Text('Code du salon : ', style: TextStyle(color: VuelColors.muted, fontSize: 13)),
                    Text(
                      _displayCode,
                      style: const TextStyle(color: VuelColors.amber, fontSize: 16, fontWeight: FontWeight.bold, letterSpacing: 2),
                    ),
                    const SizedBox(width: 8),
                    const Icon(Icons.copy_rounded, size: 16, color: VuelColors.amber),
                  ],
                ),
              ),
            ),
